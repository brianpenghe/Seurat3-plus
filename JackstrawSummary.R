...
JackstrawSummary <- function(Seurat, nrow=50, cutoff = 0.05){
	...
	JStable=Seurat@reductions$pca@jackstraw@overall.p.values[1:nrow,]
        for (i in 1:floor(npc/nrow-1)){
		JStable <- cbind(JStable,Seurat@reductions$pca@jackstraw@overall.p.values[(nrow*i+1):(nrow*i+50),])
	}
	print(JStable)
